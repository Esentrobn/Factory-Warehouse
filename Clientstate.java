...
import java.text.*;
import java.io.*;
public class Clientstate extends WareHouseUIState {
	...
	public void logout()
	{
		if ((WarehouseUIContext.instance()).getLogin() == WarehouseUIContext.IsClerk)
		{ //stem.out.println(" going to clerk \n ");
			(WarehouseUIContext.instance()).changeState(1); // exit with a code 1
		}
		else if (WarehouseUIContext.instance().getLogin() == WarehouseUIContext.IsManager)
		{  //stem.out.println(" going to login \n");
			(WarehouseUIContext.instance()).changeState(3); // exit with a code 2
		}
		else 
			(WarehouseUIContext.instance()).changeState(0); // exit code 2, indicates error
	}

	public void getClientAccount (){
		Client tempUserAccount = warehouse.searchMembership(WarehouseUIContext.instance().getUser());
		System.out.println(tempUserAccount);
	}

	private void createOrder() {
		String tempClient;
		tempClient = WarehouseUIContext.instance().getUser();
